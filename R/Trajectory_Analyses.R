...
## covariates
cov_R3STEP <- c(SD_cat, SD_num, 'ageonset', 'duponset', 'mode', 'ip_op', 'dx_b2', 'SUD', 'PAS_tot2', 'SOFAS_0', COGSTATE, SX_0, SR_1st, 'JSR_by3', 'JSR_at3', 'JSR_at6', "comp_0", "CPZ_0") %>% setdiff(c('cd8_0', YMRS)) #CAYRconv, txm0co, SUMD2_0, SUMD3_0
cov_MixReg <- c(SOFAS, SX, PSR, NSR, JSR)
cov_D3STEP <- c('SOFAS_24', SX_24, RSWG, "comp_24", "CPZ_24", "txm24co") %>% setdiff(c('cd8_24', "JSR_by3", "JSR_at3", "JSR_at6", YMRS)) #txm24co
...
plot <- plotGrowthMixtures(FINAL_model, bw = FALSE, rawdata = TRUE, time_scale = c(0, 12, 24))

plot_GBTM <- map(GBTM_models, 
                 ~ plotGrowthMixtures(.x, bw = TRUE, rawdata = TRUE, time_scale = c(0, 12, 24)))

plot_BEST <- map(list(GBTM_best, LCGA_best, GMMci_best, GMMcv_best),
                 ~ plotGrowthMixtures(.x, bw = TRUE, rawdata = TRUE, time_scale = c(0, 12, 24)))
...
## Step 8: Plot trajectories -----------------------------------------------
plot <- plotGrowthMixtures(FINAL_model, bw = FALSE,  rawdata = TRUE, time_scale = c(0, 1, 2, 3, 6, 9, 12, 18, 24))
