...
### Model specification
model1 <-  purrr::map2(reg, variance, \(x, y) c(x, y))
...
## MODEL TEST =
# test <- map_chr(head(1:k, -1), ~ glue::glue("{test}{.x} = {test}{.x+1}")) %>%
#     MplusAutomation::parseMplus(add = TRUE)
